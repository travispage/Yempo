...
if (!current_user_can($cb_user_role_permission)) {
   ...
   if ($count_forms < 2) {
      $last_form_id = $wpdb->get_var
          ...
          $wpdb->prepare
              (
              "SELECT form_id FROM " . contact_bank_contact_form() . " where form_id= %d", $form_id
          )
      );
      if (count($last_form_id) == 0) {
         $wpdb->query
             ...
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_contact_form() . "(form_id,form_name) VALUES(%d,%s)", $form_id, "Untitled Form"
             )
         );
         $wpdb->query
             (
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", $form_id, "redirect", "0"
             )
         );
         $wpdb->query
             (
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", $form_id, "redirect_url", ""
             )
         );
         $wpdb->query
             (
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", $form_id, "success_message", "Your Message has been successfully sent."
             )
         );
         $wpdb->query
             (
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", $form_id, "blank_field_message", "Required field must not be blank"
             )
         );
         $wpdb->query
             (
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", $form_id, "incorrect_email_message", "Please enter a valid e-mail address"
             )
         );
         $wpdb->query
             (
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", $form_id, "form_description", ""
             )
         );
         $settings = array();
         ...
         $wpdb->query
             ...
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_email_template_admin() . " (email_to,email_from,body_content,subject,form_id,from_name,name,send_to) VALUES(%s,%s,%s,%s,%d,%s,%s,%d)", $email_address, $email_from_email, $uxDescription_email, $email_subject, $form_id, $email_from_name, $email_name, $send_to
             )
         );
         ...
         $wpdb->query
             ...
             $wpdb->prepare
                 (
                 "INSERT INTO " . contact_bank_email_template_admin() . " (email_to,email_from,body_content,subject,form_id,from_name,name,send_to) VALUES(%s,%s,%s,%s,%d,%s,%s,%d)", "", $email_from_email, $uxDescription_email_client, $email_subject_client, $form_id, $email_from_name, $email_name_client, ""
             )
         );
      }
   }
   $form_data = $wpdb->get_results
       ...
       $wpdb->prepare
           (
           "SELECT * FROM " . contact_bank_form_settings_Table() . " where form_id = %d", $form_id
       )
   );
   ...
   $form_name = $wpdb->get_var
       ...
       $wpdb->prepare
           (
           "SELECT form_name FROM " . contact_bank_contact_form() . " where form_id = %d", $form_id
       )
   );
   ?>
                                             ...
                                             <div class="layout-control-group div_border">
                                                ...
                                                <div class="layout-controls">
                                                   <input type="text" name="ux_txt_blank_message" class="layout-span7"
                                                          value="<?php echo isset($form_settings[$form_id]["blank_field_message"]) ? esc_attr($form_settings[$form_id]["blank_field_message"]) : _e("Required field must not be blank", "contact-bank");
   ?>"
                                             ...
                                             <div class="layout-control-group div_border">
                                                ...
                                                <div class="layout-controls ">
                                                   <input type="text" name="ux_txt_incorrect_email_message" class="layout-span7"
                                                          value="<?php echo isset($form_settings[$form_id]["incorrect_email_message"]) ? esc_attr($form_settings[$form_id]["incorrect_email_message"]) : _e("Please enter a valid email address", "contact-bank");
   ?>"
