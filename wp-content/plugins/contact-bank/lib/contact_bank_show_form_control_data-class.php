      ...
      if (esc_attr($_REQUEST["param"]) == "bind_text_control") {
         ...
         if (count($control_id) != 0) {
            $form_data = $wpdb->get_results
                ...
                $wpdb->prepare
                    (
                    "SELECT * FROM " . contact_bank_dynamic_settings_form() . " where dynamicId= %d", intval($control_id)
                )
            );
            ...
            for ($flag = 0; $flag < count($form_data); $flag++) {
               if (esc_attr($form_data[$flag]->dynamic_settings_key) == "cb_dropdown_option_id" || esc_attr($form_data[$flag]->dynamic_settings_key) == "cb_dropdown_option_val" || esc_attr($form_data[$flag]->dynamic_settings_key) == "cb_checkbox_option_id" || esc_attr($form_data[$flag]->dynamic_settings_key) == "cb_checkbox_option_val" || esc_attr($form_data[$flag]->dynamic_settings_key) == "cb_radio_option_id" || esc_attr($form_data[$flag]->dynamic_settings_key) == "cb_radio_option_val") {
