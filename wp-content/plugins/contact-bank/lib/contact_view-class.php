      ...
      switch (esc_attr($_REQUEST["param"])) {
         ...
         case "delete_form":
            ...
            $control_id = $wpdb->get_results
                ...
                $wpdb->prepare
                    (
                    "SELECT control_id FROM " . create_control_Table() . " WHERE form_id = %d ", $form_id
                )
            );
            $sql = array();
            ...
            $wpdb->query
                ...
                $wpdb->prepare
                    (
                    "DELETE FROM " . contact_bank_email_template_admin() . " WHERE form_id = %d ", $form_id
                )
            );
            $wpdb->query
                (
                $wpdb->prepare
                    (
                    "DELETE FROM " . contact_bank_form_settings_Table() . " WHERE form_id = %d ", $form_id
                )
            );
            $wpdb->query
                (
                $wpdb->prepare
                    (
                    "DELETE FROM " . frontend_controls_data_Table() . " WHERE form_id = %d ", $form_id
                )
            );
            $wpdb->query
                (
                $wpdb->prepare
                    (
                    "DELETE FROM " . contact_bank_frontend_forms_Table() . " WHERE form_id = %d ", $form_id
                )
            );
            $wpdb->query
                (
                $wpdb->prepare
                    (
                    "DELETE FROM " . contact_bank_layout_settings_Table() . " WHERE form_id = %d ", $form_id
                )
            );
            $wpdb->query
                (
                $wpdb->prepare
                    (
                    "DELETE FROM " . create_control_Table() . " WHERE form_id = %d ", $form_id
                )
            );
            $wpdb->query
                (
                $wpdb->prepare
                    (
                    "DELETE FROM " . contact_bank_contact_form() . " WHERE form_id = %d ", $form_id
                )
            );
         ...
         case "submit_form_messages_settings":
            ...
            foreach ($form_settings_data as $element => $value) {
               if ($element == "form_name") {
                  $wpdb->query
                      ...
                      $wpdb->prepare
                          (
                          "UPDATE " . contact_bank_contact_form() . " SET `form_name` = %s where form_id = %d ", $value, $form_id
                      )
                  );
               } else {
                  $wpdb->query
                      ...
                      $wpdb->prepare
                          (
                          "UPDATE " . contact_bank_form_settings_Table() . " SET `form_message_value` = %s where form_message_key = %s and form_id = %d ", $value, $element, $form_id
                      )
                  );
               }
            }
            ...
            $wpdb->query
                ...
                $wpdb->prepare
                    (
                    "UPDATE " . contact_bank_email_template_admin() . " SET `body_content` = %s where form_id = %d and name = %s", $body_message, $form_id, "Admin Notification"
                )
            );
            $wpdb->query
                (
                $wpdb->prepare
                    (
                    "UPDATE " . contact_bank_email_template_admin() . " SET `email_to` = %s where form_id = %d and name = %s", "[control_" . $email_dynamicId . "]", $form_id, "Client Notification"
                )
            );
            $wpdb->query
                (
                $wpdb->prepare
                    (
                    "UPDATE " . contact_bank_email_template_admin() . " SET `send_to` = %d where form_id = %d and name = %s", 1, $form_id, "Client Notification"
                )
            );
            break;
            ...
            foreach ($form_settings as $element) {
               ...
               if ($event == "add") {
                  $wpdb->query
                      ...
                      $wpdb->prepare
                          (
                          "INSERT INTO " . create_control_Table() . "(form_id,field_id,column_dynamicId) VALUES(%d,%d,%d)", $form_id, $control_type, $id
                      )
                  );
                  ...
                  $wpdb->query
                      ...
                      $wpdb->prepare
                          (
                          "UPDATE " . create_control_Table() . " SET `sorting_order` = %d where form_id = %d and field_id = %d and column_dynamicId = %d", $dynamic_control_id, $form_id, $control_type, $id
                      )
                  );
               }
                  ...
                  if ($key == "dynamic_id" || $key == "control_type") {
                     ...
                     if ($event == "add") {
                        $wpdb->query
                            ...
                            $wpdb->prepare
                                (
                                "INSERT INTO " . contact_bank_dynamic_settings_form() . "(dynamicId,dynamic_settings_key,dynamic_settings_value) VALUES (%d,%s,%s)", $dynamic_control_id, $key, $value
                            )
                        );
                     } else {
                        $wpdb->query
                            ...
                            $wpdb->prepare
                                (
                                "UPDATE " . contact_bank_dynamic_settings_form() . " SET `dynamic_settings_value` = %s where dynamic_settings_key = %s and dynamicId = %d ", $value, $key, $controlId
                            )
                        );
                     }
                  }
         ...
         case "form_fields_sorting_order":
            ...
            $wpdb->query
                ...
                $wpdb->prepare
                    (
                    "UPDATE " . create_control_Table() . " SET `sorting_order` = CASE " . $sql . " END where form_id = %d ", $form_id
                )
            );
