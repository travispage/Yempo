   ...
   if (isset($_REQUEST["param"])) {
      if (esc_attr($_REQUEST["param"]) == "frontend_form_data") {
         ...
         $form_data = $wpdb->get_results
             ...
             $wpdb->prepare
                 (
                 "SELECT * FROM " . create_control_Table() . " WHERE form_id = %d ORDER BY sorting_order ASC", $form_id
             )
         );
         ?>
         <div id="divData">
            <table class="table table-striped" id="data-table-frontend" style="width:100%;">
                  ...
                  <tr>
                     ...
                     for ($flag = 0; $flag < count($form_data); $flag++) {
                        $form_control_labels = $wpdb->get_var
                            ...
                            $wpdb->prepare
                                (
                                "SELECT dynamic_settings_value FROM " . contact_bank_dynamic_settings_form() . " WHERE dynamicId = %d AND dynamic_settings_key = %s", intval($form_data[$flag]->control_id), "cb_label_value"
                            )
                        );
                        ?>
               ...
               <tbody>
                  ...
                  $form_submit_count = $wpdb->get_results
                      ...
                      $wpdb->prepare
                          (
                          "SELECT * FROM " . contact_bank_frontend_forms_Table() . " WHERE form_id = %d", $form_id
                      )
                  );
                     ...
                     <tr>
                        ...
                        for ($flag2 = 0; $flag2 < count($form_data); $flag2++) {
                           $form_control_labels_values = $wpdb->get_var
                               ...
                               $wpdb->prepare
                                   (
                                   "SELECT dynamic_frontend_value FROM " . frontend_controls_data_Table() . " WHERE dynamic_control_id = %d AND form_id = %d AND form_submit_id = %d", intval($form_data[$flag2]->control_id), $form_id, intval($form_submit_count[$flag1]->submit_id)
                               )
                           );
