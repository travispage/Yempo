      ...
      switch (esc_attr($_REQUEST["param"])) {
         case "email_settings":
            ...
            $email_settings = $wpdb->get_results
                ...
                $wpdb->prepare
                    (
                    "SELECT email_id,subject,name FROM " . contact_bank_email_template_admin() . " WHERE form_id = %d", $form_id
                )
            );
         ...
         case "delete_email_settings":
            ...
            $wpdb->query
                ...
                $wpdb->prepare
                    (
                    "DELETE FROM " . contact_bank_email_template_admin() . " WHERE email_id = %d ", $email_id
                )
            );
