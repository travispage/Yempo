...
if (!current_user_can($cb_user_role_permission)) {
   ...
   function create_cb_table_licensing() {
      ...
      $wpdb->query
          ...
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_licensing() . "(version, type, url) VALUES(%s, %s, %s)", "2.1.0", "Contact Bank", "" . site_url() . ""
          )
      );
   }
   ...
   if ($version == "" || $version == "1.0") {
         ...
         if (count($contact_forms) > 0) {
            for ($flag = 0; $flag < count($contact_forms); $flag++) {
               $wpdb->query
                   ...
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_contact_form() . "(form_id, form_name) VALUES(%d, %s)", intval($contact_forms[$flag]->form_id), esc_html($contact_forms[$flag]->form_name)
                   )
               );
               $wpdb->query
                   (
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", intval($contact_forms[$flag]->form_id), "redirect", esc_html($contact_forms[$flag]->chk_url)
                   )
               );
               $wpdb->query
                   (
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", intval($contact_forms[$flag]->form_id), "redirect_url", esc_html($contact_forms[$flag]->redirect_url)
                   )
               );
               $wpdb->query
                   (
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", intval($contact_forms[$flag]->form_id), "success_message", esc_html($contact_forms[$flag]->success_message)
                   )
               );
               $wpdb->query
                   (
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", intval($contact_forms[$flag]->form_id), "blank_field_message", "Required field must not be blank"
                   )
               );
               $wpdb->query
                   (
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", intval($contact_forms[$flag]->form_id), "incorrect_email_message", "Please enter a valid email address"
                   )
               );
               $wpdb->query
                   (
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", intval($contact_forms[$flag]->form_id), "form_description", ""
                   )
               );
            }
         }
               ...
               $wpdb->query
                   ...
                   $wpdb->prepare
                       ...
                       "INSERT INTO " . create_control_Table() . "(field_id,form_id,
							column_dynamicId, sorting_order) VALUES(%d, %d, %d, %d)", intval($contact_forms_controls[$flag]->field_id), intval($contact_forms_controls[$flag]->form_id), intval($contact_forms_controls[$flag]->column_dynamicId), intval($contact_forms_controls[$flag]->sorting_order)
                   )
               );
            }
         }
            ...
            $wpdb->query
                ...
                $wpdb->prepare
                    ...
                    "INSERT INTO " . contact_bank_dynamic_settings_form() . "(dynamicId,dynamic_settings_key,
						dynamic_settings_value)VALUES(%d, %s, %s)", intval($contact_forms_email_dynamic_settings[$flag]->control_id), "cb_default_txt_val", ""
                )
            );
         }
                  ...
                  $wpdb->query
                      ...
                      $wpdb->prepare
                          ...
                          "INSERT INTO " . contact_bank_dynamic_settings_form() . "(dynamicId,
							dynamic_settings_key,dynamic_settings_value) VALUES(%d, %s, %s)", intval($contact_forms_dynamic_settings[$flag]->control_id), esc_attr($contact_forms_dynamic_settings[$flag]->dynamic_settings_key), array_search($contact_forms_dynamic_settings[$flag]->dynamic_settings_key, $settings_keys_multiple_options) ? serialize(explode(";", $contact_forms_dynamic_settings[$flag]->dynamic_settings_value)) : $contact_forms_dynamic_settings[$flag]->dynamic_settings_value
                      )
                  );
               }
            }
               ...
               $wpdb->query
                   ...
                   $wpdb->prepare
                       ...
                       "INSERT INTO " . contact_bank_email_template_admin() . "(email_id, email_to, email_from,
							...
							%s, %s, %s, %d, %d, %s, %s, %s, %s, %s)", intval($contact_forms_emails[$flag]->email_id), esc_html($contact_forms_emails[$flag]->email_to), esc_html($contact_forms_emails[$flag]->email_from), esc_html($contact_forms_emails[$flag]->body_content), esc_html($contact_forms_emails[$flag]->subject), isset($contact_forms_emails[$flag]->send_to) ? esc_html($contact_forms_emails[$flag]->send_to) : "", intval($contact_forms_emails[$flag]->form_id), isset($contact_forms_emails[$flag]->from_name) ? esc_html($contact_forms_emails[$flag]->from_name) : "", isset($contact_forms_emails[$flag]->reply_to) ? esc_html($contact_forms_emails[$flag]->reply_to) : "", isset($contact_forms_emails[$flag]->cc) ? esc_html($contact_forms_emails[$flag]->cc) : "", isset($contact_forms_emails[$flag]->bcc) ? esc_html($contact_forms_emails[$flag]->bcc) : "", isset($contact_forms_emails[$flag]->name) ? esc_html($contact_forms_emails[$flag]->name) : ""
                   )
               );
            }
         }
      ...
      if (count($wpdb->get_var('SHOW TABLES LIKE "' . contact_bank_frontend_forms_Table() . '"')) == 0) {
         ...
         for ($flag = 0; $flag < count($contact_front_end_forms); $flag++) {
            $wpdb->query
                ...
                $wpdb->prepare
                    (
                    "INSERT INTO " . contact_bank_frontend_forms_Table() . "(form_id,submit_id)VALUES(%d, %d)", intval($contact_front_end_forms[$flag]->form_id), intval($contact_front_end_forms[$flag]->submit_id)
                )
            );
         }
      }
         ...
         for ($flag = 0; $flag < count($contact_front_end_forms_data); $flag++) {
            $wpdb->query
                ...
                $wpdb->prepare
					...
					dynamic_frontend_value,form_submit_id)VALUES(%d, %d, %d, %s, %d)", intval($contact_front_end_forms_data[$flag]->form_id), intval($contact_front_end_forms_data[$flag]->field_Id), intval($contact_front_end_forms_data[$flag]->control_id), esc_html($contact_front_end_forms_data[$flag]->dynamic_frontend_value), intval($contact_front_end_forms_data[$flag]->form_submit_id)
                )
            );
         }
      }
      ...
      $wpdb->query
          ...
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_contact_form() . "(form_id, form_name) VALUES(%d, %s)", 1, "Contact Form Demo"
          )
      );
      $wpdb->query
          (
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", 1, "redirect", "0"
          )
      );
      $wpdb->query
          (
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", 1, "redirect_url", ""
          )
      );
      $wpdb->query
          (
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", 1, "success_message", "Your message was sent successfully. Thanks."
          )
      );
      $wpdb->query
          (
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", 1, "blank_field_message", "Required field must not be blank"
          )
      );
      $wpdb->query
          (
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", 1, "incorrect_email_message", "Please enter a valid email address"
          )
      );
      $wpdb->query
          (
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", 1, "form_description", ""
          )
      );
      ...
      $wpdb->query
          ...
          $wpdb->prepare
              ...
              "INSERT INTO " . create_control_Table() . "(control_id, field_id,form_id,
						column_dynamicId, sorting_order) VALUES(%d, %d, %d, %d, %d)", 1, 1, 1, 1, 1
          )
      );
      ...
      $wpdb->query
          ...
          $wpdb->prepare
              (
              "INSERT INTO " . create_control_Table() . "(control_id, field_id,form_id,
						column_dynamicId, sorting_order) VALUES(%d, %d, %d, %d, %d)", 2, 3, 1, 2, 2
          )
      );
      ...
      $wpdb->query
          ...
          $wpdb->prepare
              (
              "INSERT INTO " . create_control_Table() . "(control_id, field_id,form_id,
				column_dynamicId, sorting_order) VALUES(%d, %d, %d, %d, %d)", 3, 1, 1, 3, 3
          )
      );
      ...
      $wpdb->query
          ...
          $wpdb->prepare
              (
              "INSERT INTO " . create_control_Table() . "(control_id, field_id,form_id,
				column_dynamicId, sorting_order) VALUES(%d, %d, %d, %d, %d)", 4, 2, 1, 4, 4
          )
      );
      ...
      $fields_created = $wpdb->get_results
          ...
          $wpdb->prepare
              (
              "SELECT dynamicId, dynamic_settings_value,field_id	FROM " . contact_bank_dynamic_settings_form() . " JOIN " . create_control_Table() . " ON " . contact_bank_dynamic_settings_form() . ". dynamicId  = " . create_control_Table() . ".control_id WHERE `dynamic_settings_key` = 'cb_admin_label' and form_id = %d Order By " . create_control_Table() . ".sorting_order", 1
          )
      );
      $controls = "";
      $email_dynamicId = "";
      for ($flag = 0; $flag < count($fields_created); $flag++) {
         $show_in_email = $wpdb->get_var
             ...
             $wpdb->prepare
                 (
                 "SELECT dynamic_settings_value FROM " . contact_bank_dynamic_settings_form() . " WHERE `dynamic_settings_key` = 'cb_show_email' and dynamicId = %d", intval($fields_created[$flag]->dynamicId)
             )
         );
         if ($show_in_email == "0") {
      ...
      $wpdb->query
          ...
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_email_template_admin() . " (email_to,email_from,body_content,subject,form_id,from_name,name,send_to) VALUES(%s,%s,%s,%s,%d,%s,%s,%d)", $email_address, $email_from_email, $uxDescription_email, $email_subject, 1, $email_from_name, $email_name, $send_to
          )
      );
      ...
      $wpdb->query
          ...
          $wpdb->prepare
              (
              "INSERT INTO " . contact_bank_email_template_admin() . " (email_to,email_from,body_content,subject,form_id,from_name,name,send_to) VALUES(%s,%s,%s,%s,%d,%s,%s,%d)", "", $email_from_email, $uxDescription_email_client, $email_subject_client, 1, $email_from_name, $email_name_client, ""
          )
      );
   } else if ($version == "2.0") {
      if (count($wpdb->get_var("SHOW TABLES LIKE '" . contact_bank_licensing() . "'")) == 0) {
         ...
         if (count($contact_forms_settings_table) > 0) {
            for ($flag = 0; $flag < count($contact_forms_settings_table); $flag++) {
               $wpdb->query
                   ...
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", intval($contact_forms_settings_table[$flag]->form_id), esc_attr($contact_forms_settings_table[$flag]->form_message_key), esc_html($contact_forms_settings_table[$flag]->form_message_value)
                   )
               );
            }
            for ($flag = 0; $flag < count($contact_forms_count); $flag++) {
               $wpdb->query
                   ...
                   $wpdb->prepare
                       (
                       "INSERT INTO " . contact_bank_form_settings_Table() . "(form_id,form_message_key,form_message_value)VALUES(%d, %s, %s)", intval($contact_forms_count[$flag]->form_id), "form_description", ""
                   )
               );
            }
         }
