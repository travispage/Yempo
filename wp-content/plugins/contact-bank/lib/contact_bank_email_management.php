...
if (isset($_REQUEST["param"])) {
   if (esc_attr($_REQUEST["param"]) == "email_management") {
      ...
      $frontend_control_value = $wpdb->get_results
          ...
          $wpdb->prepare
              (
              "SELECT * FROM  " . contact_bank_frontend_forms_Table() . " JOIN  " . frontend_controls_data_Table() . " ON " . contact_bank_frontend_forms_Table() . ".submit_id = " . frontend_controls_data_Table() . ".form_submit_id  WHERE " . contact_bank_frontend_forms_Table() . ".submit_id = %d", $form_submit_id
          )
      );
